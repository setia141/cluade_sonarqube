...
def _stage_fix_files(fixes: List[Dict[str, Any]]) -> None:
    """Stage only the fixed source files and their co-located test files.

    Deliberately avoids 'git add -A' so that skill working files
    (issues.json, enriched.json, fixes.json, baseline.json, validation.json,
    lang.json, test-status.json, coverage.xml, .claude/) are never committed.
    """
    staged = set()
    for fix in fixes:
        src = fix.get("file", "").strip()
        if src:
            staged.add(src)

        # Stage the matching test file if it exists and was modified
        ...
        for candidate in (
            ...
            result = subprocess.run(
                f"git ls-files --error-unmatch {candidate}",
                shell=True, capture_output=True, text=True,
            )
            if result.returncode == 0:
                staged.add(candidate)
            else:
                # New (untracked) test file — check it physically exists
                if Path(candidate).exists():
                    staged.add(candidate)

    if not staged:
        raise RuntimeError("No fix files to stage — fixes.json may be empty or missing 'file' keys.")

    for path in sorted(staged):
        print(f"  Staging: {path}")
        _run_git(f"git add {path}")
